      ...
      <el-col :span="20" :offset="1">
        <nav class="shelf">
          <router-link :to="{path:'/dataStatistics'}">
            <a class="book home-page">
              <span v-on:click="sendBreadName('数据统计')">首页</span>
            </a>
          </router-link>
...
//      '发送面包屑名'的事件
        sendBreadName(name){
//          向父组件的'current-bread'方法发送'name'数据
          this.$emit('current-bread',name);
          this.$emit('isTouchLeftNav',1);
        },
      }
  }
</script>
