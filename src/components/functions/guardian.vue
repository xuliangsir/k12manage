<template>
    <div class="picture">
        ...
        <div class="line pr">
            <el-button type="primary" @click="showDialog">邀请家长</el-button>
        ...
        <el-dialog title="邀请家长" :visible.sync="dialogVisible" :before-close="handleClose(dialogVisible)" class="dialogSzie">

            <el-form ref="form" :model="form" label-width="80px">

               <el-form-item label="学生">
                   <el-select v-model="form.kid" placeholder="许多多">
                    <el-option label="许多多" value="0"></el-option>
                    <el-option label="杨甜甜" value="1"></el-option>
                   </el-select>
               </el-form-item>

                <el-form-item label="关系">
                   <el-select v-model="form.relationship" placeholder="父亲">
                    <el-option label="父亲" value="0"></el-option>
                    <el-option label="母亲" value="1"></el-option>
                    <el-option label="爷爷" value="2"></el-option>
                    <el-option label="奶奶" value="3"></el-option>
                   </el-select>
               </el-form-item>
    ...
    export default {
        ...
        methods: {
            ...
            getData() {
                this.$http({
                    method: 'get',
                    url: 'http://k12soft.vwico.com:8080/management/guardians?schoolId=' + JSON.parse(localStorage.actor).school.id + '&studentId=' + 3, //studentId??????????  任何参数都可以
                    headers: {
                    ...
                    res.body.forEach(function(item, index, array){
                        guardianArray.push({id: item.patriarch.actor.user.id, name: item.patriarch.actor.user.username,
                            mobile: item.patriarch.actor.user.mobile, studentName: item.student.name,
                            klass: item.student.klass.name, relationship: item.relationType})
                    })
            ...
            isAdd(tag) { // 0 添加    1 取消
                ...
                if (tag === 0 && this.dialogUsage == 0) {
                   this.$http.post(
                    ...
                    'http://k12soft.vwico.com:8080/management/invitations/guardians',
                    {
                        "mobile": '13269524372',
                        "username": 'xuliang',
                        "relationType":'DADDY',
