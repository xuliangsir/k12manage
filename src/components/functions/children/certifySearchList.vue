<template>
  <div>
    <div class="coachContent">
      <el-table v-loading="loading" element-loading-text="拼命加载中" :data="certifySearchList" border style="width: 100%;border-radius: 5px;">
        ...
        <el-table-column prop="createtime" label="用户创建时间" width="150"></el-table-column>
        <el-table-column label="审核" width="120">
          <template scope="scope">
            <div v-if="scope.row.passStatus===1">
              <el-button @click.native.prevent="removeRows_and_certConfirm(scope.$index,certifySearchList,scope.row.user_id,scope.row.id);" type="text" size="small">认证通过</el-button>
              <br>
              <el-button @click.native.prevent="removeRows_and_certReject(scope.$index,certifySearchList,scope.row.user_id,scope.row.id);" type="text" size="small" style="color: #FF4949;">拒绝认证</el-button>
            </div>
            <div v-if="scope.row.passStatus===2">
              <div class="certified_status">
                <span>已认证</span>
              </div>
              <el-button @click.native.prevent="removeRows_and_certReject_while_already_confirm(scope.$index,certifySearchList,scope.row.user_id,scope.row.id);" type="text" size="small" style="color: #FF4949;">取消认证</el-button>
            </div>
          </template>
        </el-table-column>
      </el-table>
...
//        API_header = 'http://10.16.40.22:60405'
//      }
//      else
//      {
//        API_header = 'http://10.16.40.22:60405'
//      }
      this.$http({
        method:'post',
        url:this.$getApi + '/cert/search',
        params:{
          content:this.forSearchList
        },
        before(){
          this.loading = true;
        }
        ...
        this.loading = false;
        this.certifySearchList = JSON.parse(res.bodyText).data;
        this.certifySearchList.map(function (v, k) {
          if(v.url){
...
//        console.log(this.certifySearchList)
      });
    },
    props:['forSearchList'],
    data() {
    ...
    watch:{
      forSearchList:function (val,oldVal) {
        console.log('forSearchList新值:',val,'-','forSearchList旧值:',oldVal);
//        let API_header;
//        if (window.location.host === 'a.socap1.com')
//        {
////        API_header = 'http://10.163.36.39:60405'
...
//      已审核用户的审核方法
      ...
      //      移除整行申请信息&确认拒绝认证
      removeRows_and_certReject_while_already_confirm(index, rows,user_id, id) {
//        let API_header;
//        if (window.location.host === 'a.socap1.com')
//        {
////          API_header = 'http://10.163.36.39:60405'
