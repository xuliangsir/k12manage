<template>
  <div>
    <div class="coachContent">
      <el-table  v-loading="loading" element-loading-text="拼命加载中" :data="certifiedList" border style="width: 100%;border-radius: 5px;">
        ...
        <el-table-column label="审核" width="120">
          <template scope="scope">
            ...
            <el-button @click.native.prevent="removeRows_and_certReject_while_already_confirm(scope.$index,certifiedList,scope.row.user_id,scope.row.id);" type="text" size="small" style="color: #FF4949;">取消认证</el-button>
          </template>
        </el-table-column>
      </el-table>
    </div>
...
//      }
      this.$http({
        ...
        this.loading = false;
        this.certifiedList = JSON.parse(res.bodyText).data;
        this.certifiedList.map(function (v, k) {
          if(v.url){
...
//          console.log(v.real_name,v.user_id)
        });
        console.log(this.certifiedList);
      });
    },
    data() {
      return {
